...
def get_fertilizer_recommendation(disease_label, plant_type):
    """Get fertilizer recommendation ONLY from Gemini API"""
    
    if not gemini_available or not gemini_model:
        return {
            "error": "Gemini API not available. Please configure GEMINI_API_KEY.",
            "fertilizers": [],
            "application_method": "API not configured"
        }
...
Provide specific product names available in Indian agricultural market."""
        ...
        return {
            "error": f"Failed to get Gemini recommendation: {str(e)}",
            "fertilizers": [],
            "application_method": "Error occurred"
        }
